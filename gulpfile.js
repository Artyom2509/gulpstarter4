...
gulp.task('img1x', function() {
    return gulp.src('app/assets/img/_src/**/*.*')
        .pipe(imageResize({
            width: '50%'
        }))
        .pipe(imagemin())
        .pipe(gulp.dest('app/assets/img/@1x/'))
});
...
if (gulpVersion == 3) {
    ...
    var taskArr = ['grid', 'styles', 'scripts', 'browser-sync'];
    gmWatch && taskArr.unshift('img');

    gulp.task('watch', taskArr, function() {
        gulp.watch('./smartgrid.js', ['grid']);
        gulp.watch('app/' + syntax + '/**/*.' + syntax + '', ['styles']);
        gulp.watch(['libs/**/*.js', 'app/js/common.js', 'app/assets/js/add/'], ['scripts']);
...
if (gulpVersion == 4) {
    ...
    gulp.task('watch', function() {
        ...
        gulp.watch(['libs/**/*.js', 'app/assets/js/common.js', 'app/assets/js/add/'], gulp.parallel('scripts'));
        gulp.watch('app/*.html', gulp.parallel('code'));
        gmWatch && gulp.watch('app/assets/img/_src/**/*', gulp.parallel('img')); // GraphicsMagick watching image sources if allowed.
    });
    gmWatch ? gulp.task('default', gulp.parallel('grid', 'img', 'styles', 'scripts', 'browser-sync', 'watch')) :
        gulp.task('default', gulp.parallel('grid', 'styles', 'scripts', 'browser-sync', 'watch'));
